...
from .models import VideoPost, Comment, User, userdata,Membership, UserMembership, Subscription
...
def watch_video(request, video_id):
    try:
        ...
        return render(request, 'error_pg.html')
    try:
        session_obj = User.objects.get(username=request.user.username)
    except:
        messages.warning(request, 'You are not logged in to watch this video.')
    ...
    return render(request, 'video_player.html', params)
...
def profile(request, session_username):
    ...
    return render(request, 'prof_pg.html', params)
...
def dashboard(request, session_username):
    ...
    return render(request, 'creator_studio.html', params)
...
def wallet(request):
    if request.user.is_authenticated:
        def record_time(a):
            video=VideoFileClip(VideoPost.video_file)
            n1=video.duration
            frame_data=video.get_frame(1)
            img=Image.fromarray(frame_data, 'RGB')
            return n1
        def timetounits(n2):
            t=record_time(VideoPost.video_file)
            units=t//100
            return HttpResponse("The equivalent number of tokens will be:",units)
        timetounits(VideoPost.video_file)         
        return render(request,'wallet.html')
    else:
        return render(request,'error_pg.html')

def creatorprof(request, session_username):
    try:
        session_obj = User.objects.get(username=session_username)
    except ObjectDoesNotExist:
        return render(request, 'error_pg.html')
    ...
    if request.user in profile_data.subscribers.all():
        ...
        subscribed = False
    params = {'subscribed':subscribed,'session_obj':session_obj,'user_data':profile_data, 'videos': user_posts, 'sci': video_cat_science, 'blogs': video_cat_blogs, 'fashion': video_cat_fashion, 'edu':video_cat_education, 'food': video_cat_food}
    return render(request, 'profile_cr.html', params)
...
def category4(request):
    return render(request,'category4.html')
def category5(request):
    return render(request,'category5.html')
def helpnfaqs(request):
    return render(request,'helpnfaqs.html')
...
def plansnpricing(request):
    return render(request,'plansnpricing.html')
def paymentpage(request):
    return render(request,'paymentpg.html')

class MembershipView(ListView):
    model = Membership
    template_name = 'templates/plansnpricing.html'
